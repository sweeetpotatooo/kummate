...
import { useEffect, useState } from "react" // React 훅을 임포트 (useEffect, useState)
...
const RoomMate: React.FC<RoomMateSearchProps> = () => {
  ...
  useEffect(() => {
    ...
    setProfileUrl(`${API_URL}/api/${userMyprofile}`) // 사용자 프로필 API URL 설정
        ...
        <div className={styles.roomMateTitle}>
          ...
          <div className={styles.roomMateBtn}>
            ...
            <Button onClick={toggleRecruitOnly}>
              {showRecruiting ? "전체보기" : "진행중"} {/* 모집 중/전체 보기 토글 버튼 */}
            </Button>
